...
async function publishToMercadoLibre(mappedPropertyData: any, tokenInfo: any, propertyId: string) {
  try {
    // Marcar como en cola en la base de datos si tenemos propertyId
    if (propertyId) {
      try {
        ...
        console.error('Error marcando queued en DB para MercadoLibre:', dbErr)
      }
    }
    ...
    if (!publishResponse.ok) {
      ...
      // Persistir error en la DB si propertyId
      if (propertyId) {
        try {
          const payload = await getPayload({ config })
          // Mantener uploaded si existía
          const current = await payload.findByID({ collection: 'propiedades', id: propertyId })
          const keepUploaded = current?.mercadolibre?.uploaded || false
          await payload.update({
            collection: 'propiedades',
            id: propertyId,
            data: {
              mercadolibre: {
                uploaded: keepUploaded,
                externalId: current?.mercadolibre?.externalId || null,
                externalUrl: current?.mercadolibre?.externalUrl || null,
                status: 'error',
                lastSyncAt: new Date().toISOString(),
                lastError: String(errorData),
              },
            },
          })
        } catch (dbErr) {
          console.error('Error actualizando error en DB para MercadoLibre:', dbErr)
        }
      }
    ...
    // Actualizar la propiedad en la base de datos con los datos devueltos
    if (propertyId) {
      try {
        const payload = await getPayload({ config })
        await payload.update({
          ...
          data: {
            mercadolibre: {
              uploaded: true,
              externalId: String(publishedItem.id),
              externalUrl: publishedItem.permalink || null,
              status: 'ok' as const,
              lastSyncAt: new Date().toISOString(),
              lastError: null,
            },
        ...
        console.error('Error actualizando DB con datos de MercadoLibre:', dbErr)
      }
    }
    ...
    // Persistir error en DB si propertyId
    if (propertyId) {
      try {
        const payload = await getPayload({ config })
        const current = await payload.findByID({ collection: 'propiedades', id: propertyId })
        ...
        console.error('Error guardando error en DB para MercadoLibre:', dbErr)
      }
    }
